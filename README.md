...
| **speak** (8) | `send_message`, `send_embed`, `send_file`, `dm_user`, `react` | owner only |
| **manage** (23) | `pin_message`, `add_role`, `create_channel`, `set_channel_permissions`, `timeout_member` | owner only |
...
<!-- GENERATED:count -->
`do` covers all 54 registered capabilities and replaces the need for a script per action. The older single-purpose CLIs below still work and route through their original code paths.
<!-- /GENERATED:count -->
