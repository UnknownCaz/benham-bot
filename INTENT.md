...
| 24 | **One confirmation window: one hour** (2026-08-17). `conversation_ttl_seconds` 600 → 3600, matching `ttl_seconds`. Safe at tier 3 only because the tier-3 reply rule already applies — a bare "yes" cannot fire a destructive action. *(Strengthened by #37: the rule that carries this is now the token, not a name, so the hour-long window got safer rather than looser.)* |
...
| 37 | **The tier-3 confirmation token is mandatory** (2026-08-24, Tyler: *"one copy-paste action is worth an irreversible action"*). A **typed** yes on any of the seven tier-3 actions must carry the 6-hex token; naming the action no longer fires it. Supersedes the 2026-08-17 naming rule, which is kept as the revert path behind `confirm.require_token_tier3` (default on). The naming rule's guarantee was word overlap with **ordinary English** — its accepted words are channel, user and role names — so a purge parked on #general could be fired by "yeah, general seems right", and the accepted set grows silently with every capability added. A token is exact and does not rot. **Uniform across all seven, not a delete-only carve-out:** "no undo" does not pick the delete/purge/kick/ban trio anyway — a kick is an invite away and `unban_member` exists, so ban and kick are the two that DO have a repair path. **Costs no friction on the ordinary path:** the Approve button closes over the token and never reaches `read_reply`, so this binds only the typed reply — which is exactly the moment you are answering from a notification instead of looking at the preview |
| 38 | **Phase B: the Benham face runs on cazzy-mac and the PC CLI is a thin client** (2026-09-04, Tyler's D1 in `drafts\benham-phase-b\DECISIONS.md`; built 2026-09-05). The bot grows a token-gated HTTP surface on the tailnet INTERFACE address (never 0.0.0.0, host-header allowlist, `X-Benham-Token` from `~/.config/benham-bot.token`, banker-agent's exact shape), and `benham.py` on the PC becomes a client of it: **every verb keeps its name, flags, words and exit codes** - RAVEN.md, discord-proxy, `initiate.bat`, `watch-exports.ps1` and the CLAUDE.md ask-queue line are byte-identical. Outbox verbs are POST + result poll; **store verbs execute inside the bot process** - this is the queue fold-in: the bot is now the single writer of every store, the rule rooms v1 already lived by. `tests/fixtures/cli-words/` pins the words, captured BEFORE the rewrite; re-pinning one is a deliberate act named in the commit |
| 39 | **PC access is DROPPED outright** (2026-09-04, D2 - Tyler's call, **reaffirmed against the recommendation** after the consequences were named). `pc_task`, `spawn_in_room`, `codesession.py` and `watch_pc` are deleted, and `claude_agent_sdk` is needed nowhere. Consequences recorded honestly: #25 completes as deletion rather than succession; #27's spawn clause lapses (rooms keep create/list/read/post; `rooms.json`'s worker id is inert data, left in place); #15's authorized-fix half retires (auto-triage has no write session to authorize). Purpose #1 narrows: Benham is the conversation hub when Tyler is away from the PC, not a hand on it. **#34 stays code and untouched** - `rule_face_capability` refuses non-primary faces on a capability that no longer exists, and its test still passes; deleting the wall is a separate act nobody asked for. Reversible: the PC-side relay in DESIGN §2-A restores the lane without touching anything else here |
| 40 | **The exaroton token rides to the Mac** (2026-09-04, D3). `vendor/exaroton/.env` on cazzy-mac, mode 600, scp'd never git; the PC skill keeps its own copy. The token exists in two places and revocation is one rotate plus two file edits - the per-credential call CLAUDE.mac.md leaves open, made for this one |
| 41 | **Fork 4 stays closed: Raven and the initiates lane remain PC clients** (2026-09-04, D4). Their command lines and permission allowlists do not change. One rider: the initiates lane's ledger heartbeat becomes a client-side proof-of-run stamp on the PC (`state\initiative-heartbeat.local`, touched by `benham.py initiate` after a successful call) so the console's lane row stays truthful now that `initiative-log.md` is written on the Mac |
| 42 | **BOT_KEY leaves the PC after the cutover, and no stored copy exists anywhere** (2026-09-04, D5; amended 2026-09-05 on Tyler's word). Caz resets the token at the Discord Developer Portal INSIDE the down window, so the PC's string is dead before the Mac's is minted and two gateway sessions on one token cannot exist even for a second; the Mac's `config/environ.env` (600, ssh-only) is the one live copy. Revival on the PC = Reset Token + paste, after `launchctl bootout` on the Mac - the portal IS the backup |
| 43 | **`guest_off` is an owner capability, tier 2; guest ON stays a deliberate `control.json` edit** (2026-09-04, D6). Sets `guest.enabled=false` and restarts. The tray's asymmetry, kept on purpose: widening who may talk to Benham should cost more than one message. No `guest_on`, by decision |
| 44 | **`restart` is an owner capability; launchd KeepAlive is the supervisor; there is NO Mac agent** (2026-09-04, D7). The bot exits 0 on an owner-gated `restart` and launchd brings it back; `sudo launchctl kickstart -k system/com.caz.benham-bot` over ssh is the path when the API itself is wedged. Rule 4 stands: the console keeps sight, never hands, on Mac members |
| 45 | **Inbox viewer = a bot endpoint + `benham.py inbox`; the console drawer tail is a LATER Banker chip** (2026-09-04, D8) |
| 46 | **The unread-DM badge dies; Discord's own badge is the badge** (2026-09-04, D9). Not carried to the health JSON or the console tile |
| 47 | **The tray retires AT cutover, disabled not deleted** (2026-09-04, D10). `benham-bot-tray` disabled in the same step as `benham-bot`; XML backup is the revival path; `tray_bot.ps1` moves to `archive\` per #17 |
| 48 | **Wake-on-message budgets, one-in-flight locks and stuck-room detection are DEFERRED to Phase B-2** (2026-09-04, D11), after a week of Mac use. Rooms stay pull-only per #27; with #39 there is no room worker to wake, so B-2's scope is whatever session-to-session choreography rooms still need |
