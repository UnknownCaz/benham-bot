<#
supervise_bot.ps1 - keep exactly ONE benham.bot alive, and know when to stop trying.

The plain restart loop this replaces had two problems that only show up at 3am.

  A crash loop is not a crash. If the bot dies immediately - bad token, a syntax
...
overlap, and a process check so the supervisor refuses to start while a bot that
...
function Write-Log($msg) {
    ...
    try { Add-Content -Path $Log -Value $line -Encoding utf8 } catch {
        # A hand-started bot holds supervise.log exclusively (its stdout is
...
try {
    while ($true) {
        ...
        Write-Log "benham.bot exited (code $code) after ${ran}s - fast-exit streak $fastExits/$MaxFastExits"
