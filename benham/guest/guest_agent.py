...
def _system_prompt(user_id=None):
    ...
    return (base + "\n\n## Your tools on this surface\n"
            "The 'no tools on this path' section above is the chat-mode "
            "default; on THIS surface it is superseded. You have these tools, "
            "and only these:\n" + lines + "\n"
            "They touch only the caller's own workspace folder and the shared "
            "commons. Treat them as part of what you offer when you make the "
            "surface visible - guests forget these exist, so when one fits "
            "what they're doing, mention it. Everything else above still "
            "holds: no Discord actions, no reading channels, nothing on the "
            "owner's machine, and no tool "
